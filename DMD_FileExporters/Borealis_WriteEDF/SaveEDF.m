function SaveEDF(filename, data, header, type) 
...
for n=1:header.channels
    % EDF Spec indicates no filtering for Annotations channel.... 
% this function always adds annotations channel as last channel.
% so clear filter spec for last channel
...
if length(header.physmax)==1
   header.physmax(1:header.channels)=header.physmax;
elseif length(header.physmax) < header.channels
    header.physmax = [header.physmax, 1]; 
end
...
if length(header.physmin)==1
   header.physmin(1:header.channels)=header.physmin;
elseif length(header.physmin) < header.channels
    header.physmin = [header.physmin, -1]; 
end
...
if length(header.digmax)==1
   header.digmax(1:header.channels)=header.digmax;
elseif length(header.digmax) < header.channels
    header.digmax = [header.digmax, 32767];
end
...
if length(header.digmin)==1
   header.digmin(1:header.channels)=header.digmin;
   elseif length(header.digmin) < header.channels
    header.digmin = [header.digmin, -32768];
end
